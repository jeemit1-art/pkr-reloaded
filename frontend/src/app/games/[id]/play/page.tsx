      ...
      <div className="overlay" id="assignOverlay">
        <div className="modal">
          ...
          <div id="knownWrap" style={{display:'none',marginBottom:'14px'}}>
            <div style={{fontSize:'0.9rem',textTransform:'uppercase',letterSpacing:'2px',color:'var(--muted)',marginBottom:'8px'}}>Quick Select</div>
...
function buildSeats(count, tW, tH, cx, cy) {
  ...
  if (!cont) return;
  cont.innerHTML = '';
  var chipSize = Math.max(30, Math.min(tW * 0.22, 44));
  var seatGap = chipSize + 8;
...
function buildPanel(sid) {
  ...
  var totalSeats = state.game ? state.game.seats : 10;
...
window.openAssign = function openAssign(sid) {
  ...
  if (ctx && ctx.eventId) {
    pkrApi('/events/' + ctx.eventId + '/players').then(function(players) {
      renderKnownPlayers((players||[]).map(function(p){ return p.display_name; }));
    }).catch(function(){ renderKnownPlayers([]); });
  } else {
...
window.confirmAssign = async function confirmAssign() {
  ...
  state.players[sid] = Object.assign({}, existing, { name: name, transactions: existing.transactions || [] });
  saveState();
  updateSaveResultsBtn();
  window.closeAssign();
  renderTable();
...
window.openSettleUp = function() {
  ...
  showConfirmModal('💸 Settle Up?', 'Calculate final settlements for all ' + players.length + ' players.', 'Show Settlements', function() { _doOpenSettleUp(players); });
};
