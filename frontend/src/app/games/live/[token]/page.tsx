...
export default function LivePage() {
  ...
  // Render table whenever data changes
  useEffect(()=>{
    ...
    <div style={{minHeight:'100vh',background:'#060e07',display:'flex',flexDirection:'column',overflow:'hidden'}}>
      ...
      {isSettled && resultsUrl && (
        <div style={{flexShrink:0,background:'rgba(46,204,113,0.06)',
          ...
          <div style={{fontSize:12,color:'#2ecc71',fontWeight:600}}>✓ Game Settled — results locked in</div>
...
function renderTable(container: HTMLDivElement, data: any) {
  ...
  const hOff = tW/2 + seatGap;
  const vOff = tH/2 + seatGap;
  const positions: {x:number,y:number}[] = [];
  positions.push({x:cx, y:cy-vOff});
  for (let i=0;i<leftCount;i++) {
    const sp = tH/(leftCount+1);
    positions.push({x:cx-hOff, y:cy+(-tH/2+sp*(i+1))});
  }
  positions.push({x:cx, y:cy+vOff});
  for (let j=0;j<rightCount;j++) {
    const sp = tH/(rightCount+1);
    positions.push({x:cx+hOff, y:cy+(-tH/2+sp*(j+1))});
  }
  ...
  positions.forEach((_pos,i)=>{
    ...
    if (p && p.display_name) {
      const bi = (p.buy_ins||0) * (buyInAmt||0);
      const co = p.cashout||0;
      const net = p.net != null ? p.net : (co>0 ? co-bi : null);
      ...
      seat.innerHTML = `
        ...
        ${p.buy_ins>0?`<div style="font-size:clamp(0.55rem,1.4vw,0.7rem);color:rgba(201,168,76,0.6)">×${p.buy_ins}</div>`:''}
      `;
    } else {
      seat.innerHTML = `
        ...
        <div style="opacity:0.3;font-size:0.88rem;font-weight:400;color:#fff;background:rgba(0,0,0,0.4);padding:2px 7px;border-radius:5px">Seat ${seatNum}</div>
      `;
    }
