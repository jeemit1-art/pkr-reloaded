...
export default function LivePage() {
  ...
  const { game, event, players, totalIn, totalOut, bank, buy_in: buyInPrice } = data;
    ...
    <div style={{minHeight:'100vh',background:'var(--bg)',paddingBottom:80}}>
      ...
      <div style={{maxWidth:480,margin:'0 auto',padding:'20px 16px',position:'relative',zIndex:1}}>
        ...
        <div className="card">
          ...
          {sorted.map((p,i)=>{
            const buyInTotal = p.buy_ins * (buyInPrice || 0);
            const net = p.cashout!=null ? p.cashout - buyInTotal : null;
            return (
              <div key={i} style={{display:'flex',alignItems:'center',gap:12,padding:'12px 16px',
                ...
                <div style={{textAlign:'right'}}>
                  <div style={{fontSize:12,color:'var(--muted)',fontFamily:'var(--font-body),sans-serif'}}>
                    {buyInPrice ? fmt(buyInTotal) : `×${p.buy_ins}`} in
                  </div>
                  {net!=null ? (
                    ...
                    <div style={{fontSize:11,color:'var(--faint)',fontFamily:'var(--font-body),sans-serif'}}>playing</div>
                  )}
                </div>
              </div>
            );
