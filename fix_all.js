...
console.log('\n── Fix 1: Remove Cancel Game from History tab ──\n');
applyFix('Remove Cancel from History tab', EVENT_PAGE,
`                  {isHost && (
...
console.log('\n── Fix 3: Leaderboard drill-down show all games ──\n');
applyFix('Fix leaderboard drill-down filter', EVENT_PAGE,
...
applyFix('Fix net lookup in drill-down', EVENT_PAGE,
...
console.log('\n── Fix 4: Pre-set _pkrResultsSaved on refresh if already settled ──\n');
applyFix('Refresh resilience for _pkrResultsSaved', TABLE_HTML,
`    // Track whether results have been saved to PKR in this session
    window._pkrResultsSaved = false;`,
`    // Track whether results have been saved to PKR in this session
    window._pkrResultsSaved = false;

    // If game was already settled before this page load (e.g. after a refresh),
    // pre-set the flag so End Game is not blocked
    (async function() {
      var ctx = getPkrCtx();
      ...
      try {
        ...
        if (game && game.status === 'settled') {
          ...
          if (saveBtn) { saveBtn.innerHTML = '<span class="game-tab-icon">\\u2705</span>Saved!'; saveBtn.disabled = true; saveBtn.style.display = ''; }
...
console.log('\n' + '─'.repeat(50));
console.log(`\n✅ ${totalFixed} fix(es) applied, ⚠️  ${totalSkipped} skipped.`);
console.log('\nNext steps:');
console.log("  git add 'frontend/src/app/events/[id]/page.tsx' frontend/public/table.html");
console.log('  git commit -m "fix: members tab, cancel game, leaderboard, refresh resilience"');
